...
const setRoleController = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { userId, role } = req.body;

    // Validate required fields
    if (!userId || !role) {
        return next(new AppError(httpStatus.BAD_REQUEST, "User ID and role are required"));
    }

    // Validate role is either USER or AGENT
    if (![Role.USER, Role.AGENT].includes(role)) {
        return next(new AppError(
            httpStatus.BAD_REQUEST,
            "Invalid role. Only USER or AGENT roles are allowed"
        ));
    }
